...
class SubsetTrainer(BaseTrainer):
    def __init__(
        ...
        if args.dataset == "svhn":
            ...
            self.train_target = self.train_dataset.dataset["label"].cpu().detach().numpy()
        else:
    ...
    def _select_from_subset(self) -> np.ndarray:
        ...
        for c in np.unique(self.train_target):
            class_indices = np.intersect1d(np.where(self.train_target == c)[0], self.unique)
            indices_per_class = np.random.choice(class_indices, size=int(np.ceil(self.datasize * self.args.train_frac / self.args.num_classes)), replace=False)
